...
def build_instruction_tree(instructions,
    ...
    for part in unchecked_opc:
        opcs = [inst.get_opcode_part(part) for inst in instructions]
        if len(set(opcs)) > 1:
            opc = part
            cur_node.opcode = opc
            break
...
"{size_info}".format(
    ...
    def gen_qom_model(self, gen_files, generator):
        ...
        for reg in self.regs:
            var = Variable(reg.name, Type["TCGv"])
            reg_vars.append((reg, var, None))
            transl_c.add_global_variable(var)
            gen_transl.add_global_variable(Variable(reg.name, Type["tcg"]))
...
"argument type than before" % (n, xstr(v[0]), v[1])
        ...
        self.instr_tree_root = build_instruction_tree(self.instructions, InstructionNode())
    ...
    def gen_all(self, qemu_src,
        ...
        old_path = join(qemu_src, path)
        new_path = join(qemu_src, path[:-1] + "i3s.c")

        rename(old_path, new_path)
        if with_chunk_graph:
            rename(old_path + ".chunks.gv", new_path + ".chunks.gv")

        with open(join(qemu_src, old_path), "w") as f:
            f.write("""
