        ...
        if ins is None:
            ...
            for key, node in opc_dict.items():
                if key.isdigit():
                    key += shift_str
                    sc = SwitchCase(
                        CINT(self.tcg.byte_swap(int(key, base = 2), swap_size),
                            base = 16
                        )
                    )
                    cases.append(sc)
...
"Didn't find proper var for field at offset %d in %s" % (
    ...
    def gen_val_byte_num(self,
        ...
        byte_num = (ceil(byte_num * 1.0 / self.cpu.read_size) *
            self.cpu.read_size
        )
        assert(byte_num <= 8)
            ...
            for m in finditer("<(.+?)>|([^<>]+)",
                ...
                try:
                    ...
                    if v[1] is None:
                        ...
                        opers_call.append(Call(v[1], var))
                except KeyError:
                    param = m.group(0)
                format_line += param
