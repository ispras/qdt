        ...
        if access in reg.access:
            ...
            case.add_child(
                OpCall(
                    ...
                    OpMCall("stderr"),
                    OpStrConcat(
                        CSTR("%%s: %s 0x%%0%d" % ("Reading from" if access == "r" else "Writing to", digits)),
                        OpMCall("HWADDR_PRIx"),
                        CSTR("\\n")
                    ),
                    OpMCall("__FUNCTION__"),
                    offset_name
                )
            )
...
""".format(
    ...
    def gen_mmio_read(name, struct_name, type_cast_macro, regs = None):
        ...
        switch = BranchSwitch(func.args[1], cases = cases)
        switch.bodies["default"].add_child(
            OpCall(
                "printf",
                OpStrConcat(
    ...
    def gen_mmio_write(name, struct_name, type_cast_macro, regs = None):
        ...
        s = Type.lookup(struct_name).gen_var("s", pointer = True)
        cases = gen_reg_cases(regs, "w", func.args[1], func.args[2], None, s)
