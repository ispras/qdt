...
class ConfigHost(object):
    ...
    def __init__(self, config_host_content):
        self.chc = config_host_content

    def __getattr__(self, parameter):
        return fixpath(QemuVersionDescription.ch_lookup(self.chc, parameter))
...
class QemuVersionDescription(object):
    ...
    def __init__(self, build_path, version = None):
        ...
        with open(config_host_path) as f:
            config_host = ConfigHost(f.read())

        self.src_path = config_host.SRC_PATH
        self.target_list = config_host.TARGET_DIRS.split(" ")
        self.bindir = join(config_host.prefix, "bin")
