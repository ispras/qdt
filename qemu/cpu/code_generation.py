...
def fill_decode_opc_body(cputype, function, cpu_env):
    ...
    def decode_opc_epilogue(node, instruction, operands, total_read, text):
        operands_to_args = [copy(o) for o in operands]
        cputype.name_shortener(operands_to_args, text)
...
def fill_print_insn_body(cputype, function):
    ...
    def gen_disas_opcode_read(node, val, offset, length):
        offset //= BYTE_BITSIZE
        byte_length = length // BYTE_BITSIZE
        addr = function.args[0]
    ...
    ParseTreeCodeBuilder(cputype, body, gen_disas_opcode_read,
        print_ins_epilogue, default_switch_case_nodes,
        add_break = False
    )
