...
def build_instruction_tree(node, instructions, read_bitsize,
    ...
    for infix, infix_instructions in sorted(iid.items()):
        ...
        build_instruction_tree(n, infix_instructions, read_bitsize,
    ...
    merged_subtree = []
    default_subtree_node = subtree.get("default", None)
    ...
    for infix, subtree_node in subtree.items():
        if (    default_subtree_node is not None
            ...
            continue
        for merged_subtree_node, infixes in merged_subtree:
            if merged_subtree_node == subtree_node:
                infixes.append(infix)
                break
        else:
            merged_subtree.append((subtree_node, [infix]))
    ...
    if default_subtree_node is not None:
        ...
        # Use "default" case for subtree with greatest count of infixes.

        # maximum count of subtree infixes
        max_csi = max(
            len(infixes) for _, infixes in merged_subtree
        )
        # count of maximums
        count_max_csi = sum(
            len(infixes) == max_csi for _, infixes in merged_subtree
        )
        default_infixes_count = all_infixes_count - used_infixes_count

        if default_infixes_count > max_csi:
            merged_subtree.append((default_subtree_node, ["default"]))
        else:
            used_infixes = set()

            for _, infixes in merged_subtree:
                for infix in infixes:
                    used_infixes.add(int(infix, base = 2))

            interval_len = interval[1]
            default_infixes = sorted(
                list("{0:0{1}b}".format(infix, interval_len) for infix in
                     set(range(all_infixes_count)) - used_infixes
                )
            )

            for i, (_, infixes) in enumerate(merged_subtree):
                if infixes[0] < default_infixes[0]:
                    continue
                merged_subtree.insert(i,
                    (default_subtree_node, default_infixes)
                )
                break
            else:
            ...
            if default_infixes_count < max_csi and count_max_csi == 1:
                for i, (_, infixes) in enumerate(merged_subtree):
                    if len(infixes) == max_csi:
                        merged_subtree_node, _ = merged_subtree.pop(i)
                        merged_subtree.append(
                            (merged_subtree_node, ["default"])
                        )
                        break

    if (    len(merged_subtree) == 1
        and (   merged_subtree[0][1][0] == "default"
             or len(merged_subtree[0][1]) == all_infixes_count
        )
    ):
        subtree_node = merged_subtree[0][0]
        ...
        for merged_subtree_node, infixes in merged_subtree:
            if infixes == ["default"]:
                node.subtree["default"] = merged_subtree_node
            else:
