...
def fill_tree_reading_seq(node, read_bitsize, already_read = 0):
    ...
    if ins is None:
        offset, length = node.interval

        reading_seq = calc_node_reading_seq(offset + length, already_read,
            node.limit_read
        )
        ...
        node.reading_seq = calc_node_reading_seq(len(ins), already_read,
            node.limit_read
        )
...
class CPUType(QOMCPU):
    __attribute_info__ = OrderedDict([
        ...
        ("target_long_bits", {
            "short": _("Log2 of target long size in bytes"),
            "input": int
        }),
        ("target_page_bits", {
    ...
    def co_gen(self, src,
        ...
        **_
        ...
        self.read_bitsize = info.read_size * BYTE_BITSIZE
        self.reg_types = info.reg_types
        self.name_shortener = info.name_shortener

        if self.read_bitsize not in SUPPORTED_READ_BITSIZES:
            raise RuntimeError(
        ...
        for i in self.instructions:
            ...
            i.split_operands_fill_offsets(self.read_bitsize)
        ...
        try:
            ...
            min_len = 0
            max_len = 0
        instr_len_is_fixed = min_len == max_len

        self.min_instr_len = min_len

        print("{arch} is a {endianess}-endian {is_fixed} instruction length"
            " architecture with {len_info}".format(
            arch = self.target_name.upper(),
            endianess = "big" if self.target_bigendian else "little",
            is_fixed = "fixed" if instr_len_is_fixed else "variable",
            len_info = ("length = %d bits" % min_len if instr_len_is_fixed else
...
""")
    ...
    def _co_gen_target_code(self, src):
        if self.instructions:
            n = build_instruction_tree_root(self.instructions,
                self.read_bitsize
            )
            fill_tree_reading_seq(n, self.read_bitsize)
            self.instruction_tree_root = n
        else:
