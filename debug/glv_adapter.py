...
class Fname2Chunks(dict):
    """ This class is file name to git diff chunks trie. """
    ...
    def __gen_dict__(self, obj, gen):
        for attr, val in obj.items():
            ...
            if isinstance(val, dict):
                # Gen nested dict.
                gen.write('{')
                self.__gen_dict__(val, gen)
                gen.write("},")
            else:
                value, prefix = val
                diff, rename = value
                gen.write("((")
                if isinstance(diff, intervalmap):
                    diff.__gen_code__(gen)
                else:
                    gen.pprint(diff)
                gen.write(',')
                gen.pprint(rename)
