...
### TCG front-end tester tests

Tests for TCG front-end tester are C programs with comments containing special
commands, that are executed by python interpreter. These commands specify
breakpoints positions and names of variables for comparison. Supported commands
are declared in `c2t/command_executor.py` There are simple tests for
arithmetic, logic, bitwise, memory (load/store) and branch instructions that
are located in `c2t/tests`.
