...
- Automated line number adjustment for breakpoint positions based in Git
history analysis.
...
differ) while using of `int32_t` ot `int64_t` is not.
...
C2T).
It's work is configures using both a configuration file (`c2t/configs`
folder`) and CLI.
