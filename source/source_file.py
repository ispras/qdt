...
class TypeFixerVisitor(TypeReferencesVisitor):
    ...
    def on_visit(self):
        if isinstance(self.cur, Type):
            ...
            # The `t`ype is provided by an inclusion.
            if t.name in self.source.types:
                return
    ...
    def visit(self):
        ...
        if s.locked:
            ...
            # adds them to its references them.
            ...
            # The current approach does not generate header inclusion chunks
            # for foreign types which are contained in the `references` list.
