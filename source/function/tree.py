...
class DeclarationInChildrenSearcher(NodeVisitor):
...
class StrConcat(CNode):
    ...
    def __c__(self, writer):
        writer.join(self.delim, self.children, "__c__")
    ...
    def add_child(self, child):
        ...
        if self.children:
            ...
            if (   v.full_deref != child.full_deref
                or v.static != child.static
                or v.const != child.const
            ):
                raise TypeError(
...
class MCall(SemicolonPresence):
    ...
    def __c__(self, writer):
        ...
        if self.children:
            writer.write("(@a")
            writer.join(",@s", self.children, "__c__")
            writer.write("@c)")
    ...
    def __init__(self, value, field):
        ...
        try:
            if struct.definition:
                struct.definition.fields[field]
            else:
                struct.fields[field]
        except:
