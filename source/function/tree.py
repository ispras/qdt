...
class Node(object):
    ...
    def add_child(self, child):
        if isinstance(child, str):
           ...
           or py_version[0] == 2 and isinstance(child, long)
        ):
            child = CINT(child)
...
class Return(Node):
    ...
    def __init__(self, arg = None):
        super(Return, self).__init__(children = [arg])
        if arg:
...
class LoopFor(Node):
    ...
    def __init__(self, init, cond, step):
...
class SemicolonPresence(Node):
    """ SemicolonPresence class is used for determine semicolon presence
    during the output
    """
...
class Call(SemicolonPresence):
    ...
    def __init__(self, func, *args):
        ...
        if isinstance(func, str):
            ...
            t = func.children[0]
            self.prefix = t.name + func.delim
            while isinstance(t, OpSDeref):
                func = t
                t = t.children[0]
                self.prefix += t.name + func.delim
            self.prefix += func.field + "(@a"
        else:
            raise ValueError(
                "Invalid type of func in Call: {}".format(type(func))
            )
...
class Declare(SemicolonPresence):
    ...
    def __c__(self, writer):
        ...
        for child in self.children[1:]:
            ...
            writer.write(self.delim + asterisks)
            child.__c__(writer)
