...
""")
    ...
    def _on_include(self, includer, inclusion, is_global):
        if not inclusion in self.reg_header:
            print("Parsing " + inclusion + " as inclusion")
            h = Header(path = inclusion, is_global = is_global)
            h.parsed = True
        else:
            h = self.header_lookup(inclusion)

        self.header_lookup(includer).add_inclusion(h)

    def _on_define(self, definer, macro):
        # macro is ply.cpp.Macro
        if "__FILE__" == macro.name:
            return

        h = self.header_lookup(definer)

        if macro.arglist == None:
            try:
                m = self.type_lookup(macro.name)
                if not m.definer.path == definer:
                    print "Info: multiple definitions of macro %s in %s and %s"\
                         % (macro.name, m.definer.path, definer)
            except:
    ...
    def _build_inclusions_recursive(self, start_dir, prefix):
        full_name = os.path.join(start_dir, prefix)
        if (os.path.isdir(full_name)):
            for entry in os.listdir(full_name):
                self._build_inclusions_recursive(
                    start_dir,
                    os.path.join(prefix, entry)
                )
        else:
            (name, ext) = os.path.splitext(prefix)
            if ext == ".h":
                if not prefix in self.reg_header:
                    h = Header(path = prefix, is_global = False)
                    h.parsed = False
                else:
                    h = self.header_lookup(prefix)
                ...
                if not h.parsed:
                    ...
                    p.on_include = self._on_include
                    p.on_define.append(self._on_define)
    ...
    def build_inclusions(self, dname):
        for h in self.reg_header.values():
            h.parsed = False

        for entry in os.listdir(dname):
            self._build_inclusions_recursive(dname, entry)
