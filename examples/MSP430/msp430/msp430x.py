...
#     add_child: if child isinstance GeneratorType enroll it
#     []: OpIndex(self, arg)
...
class FII(object):
    ...
    def __call__(self, sem):
        append_FII(self.opcode, sem.__name__.lower(), self.has_ext, sem,
            self.changes_dst, self.sub_sp, self.save_pc,
            comment = sem.__doc__,
        )
        return sem
...
def append_FI(opcode, base_name, semantics, changes_dst, reads_dst, **kw):
    ...
    for ext in ((True, False) if with_ext else (False,)):
        ...
        if ext:
            ...
            name = base_name
            ext_word = []
            fmt_bwa = ".<bw>" # al field exists in exstenstion word only
        ...
        for ad in (
            ...
            def src_imm_sem(f, s, ad = ad, ext = ext):
                src_val = Type["tcg"]("src_val")
                tcg_mem_size = Type["TCGMemOp"]("size")
                mem_addr = None
            ...
            fields = ext_word + [
                ...
                c(1, ad),
                o(1, "bw"), # B/W
                c(2, 1), # As == 01, indexed/symbolic/absolute
                o(4, "dst"),
                o(16, "soff"),
            ] + dst_offset
            ...
            fields = ext_word + [
                ...
                c(1, ad),
                o(1, "bw"), # B/W
                o(2, "as"), # As == (0 or 10b) or (As == 11b and src != 0)
                o(4, "dst"),
            ] + dst_offset
            ...
            def src_reg_sem(f, s, ad = ad, ext = ext):
                ...
                if ad:
                    ...
                    yield BranchIf(OpLE(2, _as))(
                        *flat_list(gen_set_mem_size)(f, s, True, ext,
                            tcg_mem_size
                        )
                    )
    ...
    for ext in [False] + ([True] if (has_ext and with_ext) else []):
        ...
        if ext:
            ext_word = [
                ...
                # XXX: zeros are expected here but we can't append 00 to the
                # opcode because of CPU gen limitation. It detects register
                # mode extension word with zc == rep == 0 as as this
                # non-register mode extension word. And corresponding
                # X-instruction is considered illegal...
                o(2, "z"),
        ...
        fields = ext_word + [
            c(9, opcode),
            o(1, "bw"), # B/W
            c(2, 1), # Ad == 01, indexed/symbolic/absolute
            o(4, "dst"), # Rdst
            o(16, "doff"),
        ]
        ...
        def dst_reg_sem_iteration(f, s, ext, bits, tcg_mem_size):
            ...
            if sub_sp:
                yield BranchIf(
                    ...
                    *flat_list(gen_autoincrement)(f, s, dst,
            ...
            if changes_dst:
                yield OpCombAssign(res, bits[1], "&") # mask
                if_ad_body = flat_list(gen_set_dst_code)(f, s, True, res,
                    tcg_mem_size, mem_addr
                )
                if_ad_body.append(
                    BranchElse()(
                        *flat_list(gen_set_dst_code)(f, s, False, res,
                            tcg_mem_size,
                            mem_addr
                        )
                    )
                )
                yield BranchIf(ad)(*if_ad_body)
        ...
        def dst_reg_sem(f, s, ext = ext):
            ...
            yield BranchIf(ad)(
                *flat_list(gen_set_mem_size)(f, s, True, ext, tcg_mem_size)
            )
            ...
            if ext:
                ...
                yield Comment("At least one repetition")
                yield OpInc(reps)

                yield LoopWhile(OpDec(reps))(
                    *flat_list(dst_reg_sem_iteration)(
                        f, s, ext, bits, tcg_mem_size
                    )
                )
            else:
        ...
        fields = ext_word + [
            c(9, opcode),
            o(1, "bw"), # B/W
            o(2, "ad"), # Ad == (0 or 10b) or (Ad == 11b and dst != 0)
            o(4, "dst"), # Rdst
        ]
...
def gen_define_size_bits(f, s, ext, bits):
    int = Type["int"]
    bits[:] = msb, mask, carry = int("msb"), int("mask"), int("carry")

    bw = f["bw"]

    bw_size = BranchIf(bw)(
        OpAssign(msb, CINT("0x80")),
        OpAssign(carry, CINT("0x100")),
        BranchElse()(
            OpAssign(msb, CINT("0x8000")),
            OpAssign(carry, CINT("0x10000")),
        )
    )
...
def gen_sub_sp(f, s, ext, msb, mask, carry):
    ...
    if ext:
        yield BranchIf(OpOr(OpEq(msb, "0x80"), OpEq(msb, "0x8000")))(
...
def gen_set_mem_size(f, s, ad, ext, tcg_mem_size):
    ...
    bw_size = BranchIf(bw)(
        OpAssign(tcg_mem_size, Type["MO_UB"]),
        BranchElse()(
            OpAssign(tcg_mem_size, Type["MO_UW"])
        )
    )
    ...
    if ext:
        yield BranchIf(f["al"])(
            ...
            BranchElse()(
                BranchIf(bw)(
                    Comment("address size mode"),
                    OpAssign(tcg_mem_size, Type["MO_UL"]),
                    BranchElse()(
                        Comment("reserved")
                    )
                )
...
def gen_get_operand_idx_code(f, s, ext, operand, op_off, val, tcg_mem_size,
    ...
    if oper_mem_addr is None:
        oper_mem_addr = Type["tcg"]("oper_mem_addr")
...
def gen_get_oper_reg_code(f, s, ext, mode, operand, oper_val, tcg_mem_size,
    ...
    if deferred_increment:
        ...
        autoincrement = BranchIf(OpEq(mode, 3))(
            *flat_list(gen_autoincrement)(f, s, operand,
    ...
    yield BranchIf(OpLogAnd(OpEq(operand, 2), OpEq(mode, 2)))(
        ...
        BranchElse()(
            ...
            *flat_list(gen_truncate_val)(f, s, oper_val, ext)
        )
    )
...
def set_sr_flag_if(f, s, flag, cond):
    ...
    return BranchIf(cond)(
        OpAssign(sr, OpOr(sr, MCall(flag))),
        BranchElse()(
...
def set_sr_flag(f, s, flag):
    sr = SR(f, s)
    return OpAssign(sr, OpOr(sr, MCall(flag)))
...
def gen_reti_430x(f, s):
    ...
    pc_19_16_and_sr = Type["tcg"]("pc_19_16_and_sr")
...
def read_src_indirect(f, s, src_val):
    ...
    src_mem_addr = Type["tcg"]("src_mem_addr")

    yield BranchSwitch(src)(
        SwitchCase(2)(
            Comment("CG1, There is no As but let it be 10 (indirect)"),
            OpAssign(src_val, CINT("0x00004"))
        ),
        SwitchCase(3)(
            Comment("CG2, There is no As but let it be 10 (indirect)"),
...
def read_src_autoincrement(f, s, src_val):
    ...
    yield BranchSwitch(src)(
        ...
        SwitchCase(2)(
            Comment("CG1, There is no As but let it be 11 (autoincrement)"),
            OpAssign(src_val, CINT("0x00008"))
        ),
        SwitchCase(3)(
            Comment("CG2, There is no As but let it be 11 (autoincrement)"),
...
def write_dst_absolute(f, s, dst_val):
    dst_mem_addr = Type["tcg"]("dst_mem_addr")
    yield OpAssign(dst_mem_addr, f["imm"])

    yield Call("tcg_gen_qemu_st_tl", dst_val, dst_mem_addr, 0,
        OpOr(Type["MO_UL"], Type["MO_TE"])
    )
...
def append_common_instructions():
    ...
    def BIS(f, s, src, dst, res, *bits):
        ...
        yield OpAssign(res, OpOr(src, dst))

    @FI(0xB, changes_dst = False)
    ...
    def DADD(f, s, src, dst, res, msb, mask, carry):
        ...
        # BCD arithmetics
        carry_outs = Type["tcg"]("carry_outs")
        actual_carry_outs = Type["tcg"]("actual_carry_outs")
    ...
    FI(4, reads_dst = False)(MOV)

    # NOP -> MOV #0, R3

    # POP -> MOV @SP+, dst

    @FII(((0x10 + 2) << 1) | 0, changes_dst = False, sub_sp = True)
    def PUSH(f, s, dst, res, instruction_size, ext, msb, mask, carry):
        ...
        if ext:
            yield BranchIf(OpEq(msb, "0x80"))(
                Call("tcg_gen_qemu_st_tl", dst, sp, 0,
                     OpOr(MCall("MO_UB"), MCall("MO_TE"))
                ),
    ...
    def XOR(f, s, src, dst, res, msb, mask, carry):
        ...
        yield OpAssign(res, OpXor(src, dst))
        yield set_neg(f, s, res, msb)
        yield set_zero(f, s, res, mask)
        ...
        #    reset otherwise
        set_sr_flag_if(f, s, "SR_C",
            OpLogAnd(OpAnd(src, msb), OpAnd(src, msb))
        )
...
def gen_msp430x_instructions():
    ...
    def popm(f, s):
        ...
        reg_n = Type["int"]("reg_n")
        last_reg = Type["int"]("last_reg")
        sp = SP(f, s)
        regs = s["regs"]
        reg_val = Type["tcg"]("reg_val")
        ...
        yield LoopFor(None, OpLE(reg_n, last_reg), OpInc(reg_n))(
            BranchIf(f["aw"])(
                Call("tcg_gen_qemu_ld_tl", reg_val, sp, 0,
                    OpOr(MCall("MO_UW"), MCall("MO_TE"))
                ),
                OpCombAssign(sp, 2, "+"),
    ...
    def pushm(f, s):
        ...
        yield LoopFor(None, OpLE(last_reg, reg_n), OpDec(reg_n))(
            ...
            BranchIf(f["aw"])(
                OpCombAssign(sp, 2, "-"),
                Call("tcg_gen_qemu_st_tl", reg_val, sp, 0,
                    OpOr(MCall("MO_UW"), MCall("MO_TE"))
                ),
    ...
    def RLAM(f, s, dst_val, imm, res):
        ...
        mask = Type["uint32_t"]("mask")

        msb = Type["uint32_t"]("msb")
...
def reg_types(s, ext):
    ...
    extend_offset = Function(
        name = "extend_offset",
        ret_type = Type["uint32_t"],
        args = [Type["uint32_t"]("offset")],
        static = True,
        inline = True,
    )
...
def print_indexed(func, module):
    ...
    yield BranchSwitch(reg)(
        ...
        SwitchCaseDefault()(
            Comment("indexed"),
            fpr("0x%lx(%s)", OpIndex(module["regs"], OpSub(reg, 1)))
        )
    )
...
def print_src(func, module):
    ...
    yield BranchSwitch(as_)(
        ...
        SwitchCaseDefault()(
            fpr("; error: As==%ld must not be handled here", as_)
        )
    )
