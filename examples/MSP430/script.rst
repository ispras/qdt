#####################################################
Добавление аритектуры процессора и периферии в Qemu \
...
окружении и ряд вспомогательных программ.
Однако здесь содержатся инструкции по использованию необходимых компонентов,
загружаемых по отдельности.
...
Следующее будет полезно в случае выполнения продвинутых действия на
...
*Если кэш не был построен в графическом редакторе ранее, данное действие может
...
- грубое тестирование, основанное на проверки логики работы программы
  на уровне языка Си;
...
программ) должна
одинаково работать независимо от вычислителя (т.е. должен быть пройден
точно такой же путь выполнения, должны совпадать значения в переменных и
т.п.).
Таким образом, если скомпилировать программу под основною машину (напр.,
AMD64) и проверяемую.
А затем запустить под отладчиком, контролируя вычислительный процесс на
абстракции языка Си (значения переменных, номера строк выполняемых
инструкций), то, в случае корректной реализации проверяемой архитектуры,
выполнение не должно иметь наблюдаемых отличий.
...
написанного на ассемблера, на настоящем МК.
...
являются вычисляемыми выражениями на языка Python (в данном случае строками).

В случае использования файлов из Energia IDE, вместо ``MSP430_SUPPORT`` и
``MSP430_TOOLCHAIN`` следует указать путь к папке, содержащей исполняемый
файл ``energia`` (переменная ``ENERGIA_PATH``).
Напр., ``"$(pwd)/energia-1.8.10E23-linux64/energia-1.8.10E23"``.
Сценарий  ``msp430_test.py`` автоматически найдёт пути до требуемых
файлов и папок.
...
терминала Qemu, подключенного к UART МК (Ctrl-Alt-2 или через меню
"View" (если Qemu собран с GTK)).

Сборку скетча следует производить, выбрав правильный МК:
...
"Показывать подробный вывод" в "Файл > Настройки".

Текущая реализация модели msp430x2xx является неполной.
Ввиду чего запуск многих других промеров из Energia IDE будет безуспешным.
