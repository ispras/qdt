...
:param caches:
    ...
    algorithm testing is not required.
        ...
        self.qvc = "qvc*_%s.py" % qemugit.commit(project.target_version).hexsha
    ...
    def __enter__(self):
        ...
        if self.caches is not None:
            for cache in glob(join(self.caches, self.qvc)):
    ...
    def __measure__(self, ctx):
        ...
        for cache in glob(join(ctx.tmp_build, ctx.qvc)):
