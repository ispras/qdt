...
4. Сравнения: равенство (==), неравенство (!=), больше (>), меньше (<), больше
...
То, что возможно покрыть имеющимися на момент написания работы тестами.
...
`mov`), `inc` (инкрементировать, реализуется инструкцией `add`) и `ret`
...
Имеется возможность и покрытия тестированием инструкций, о которых было сказано
[$](rel.cover_addge).
...
отдельной трудной задачей и выходит за рамки данной работы.
