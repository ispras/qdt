...
существующих решениях <!--вроде не нужно выделять--> пишутся на языке ассемблера или генерируются в виде
машинного кода, то есть являются архитектурно-зависимыми. <!--может и не так исправлять, но как-то режет слух-->
...
Обычно <!--вроде не нужна запятая--> при тестировании TCG виртуальные устройства не представляют интереса.
...
на Си (они входят в состав QDT). <!--может быть в состав c2t?-->
...
Те, которые возможно покрыть тестами, имеющимися на момент написания работы. <!--не уловил, к чему относятся "Те"-->
...
Имеется возможность покрыть тестированием инструкции, о которых было сказано
[$](#rel.cover_addge).
...
Например, для архитектуры ARM32 инструкция `bic` очистки определённых битов.
...
Данный факт делает неприменимыми эти решения при отсутствии такого эталона.
