...
В этом случае придется решать задачу разработки виртуальной машины,
...
машины, на которой ведется разработка (заменитель эталона).
...
Главная проблема заключается в том, у процессора другой архитектуры
...
Суть предлагаемого подхода заключается в повышения уровня представления
...
Этого предлагается достичь за счет написания тестовых сценариев на языке
...
![Рис. <a name="pic.Approach">$</a>. Схема осуществления тестирования
](approach.png)
...
версия Qemu.
Подобный случай, строго говоря, входит за рамки тестирование фронтенда TCG.
...
![Рис. <a name="pic.System">$</a>. Схема проведения тестирования
](system.png)
...
Поддержка RSP фронтендом TCG не является обязательным (в техническом смысле)
при реализации процессорной архитектуры Qemu.
С практической точки зрения, RSP реализуют всегда.
При этом её сравнительно просто добавить.
...
Табл. <a name="tbl.sw_versions">$</a>. Версии использованного ПО
...
(`-singlestep`), с включенной отладочной трассировкой и выключенными
...
гостевой блок блоками, получая подпрограммы
...
его непосредственно.
Посоле выполнения каждой подпрограммы происходит возврат в служебный код
...
Из-за сцепления блоков, предотвращающим возврата в служебный код, в трассу
не попадают гостевые адреса инструкций в прицепленных блоков.
...
Табл. <a name="tbl.isa_cover_num">$</a>. Количество покрытых инструкций
...
Табл. <a name="tbl.isa_cover">$</a>. Список покрытых/не покрытых инструкций
