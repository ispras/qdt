...
abstract: |
  ...
  **Аннотация.**
  ...
  Подход реализован в программном средстве c2t (CPU Testing Tool).
  Данное средство входит в
...
VirtualBox и VMware.
Основные отличия от EmuFuzzer заключаются в том, что тестируются полносистемные
эмуляторы и что в
...
различных процессорных архитектур использует, подобно другим подходам,
сравнение с
эталоном.
...
реализации компилятора (undefined behaviour и implementation-defined
behaviour).
...
Из этого следует, что подход главным образом ориентирован на проверку
...
Другими словами, комментарии указывают средству c2t места, на которые
необходимо
...
Например, архитектура MSP430 имеет 7 режимов адресации.
...
Для проверки последнего используются тесты, содержащие вызовы функций с большим
количеством параметров.
Это гарантирует размещение входных параметров в стеке, т.к. многие
...
Представлено отношение количества покрытых ветвей конструкций `switch` к
общему количеству ветвей, содержащих семантику уникальных инструкций.
Данный результат также соответствует отношению количества выполненных Qemu
инструкций к общему количеству инструкций тестируемой гостевой процессорной
...
А именно только те инструкции, которые возможно покрыть тестами, имеющимися
на момент написания
работы.
Из полученного результата исключены инструкции, которые предназначены для
удобства написания
...
ошибки, так и неумышленные ошибки.
