...
в неё поддержку RSP, как сказано [$](#rel.RSPNeed).
...
<!-- больше слов про 6% на ARM -->
Общее количество инструкций ARM32 было подсчитано без учёта суффиксов c, f, s,
x.
Полученное количество обусловлено наличием практически у всех вошедших в
подсчет инструкций 15 различных вариаций.
Эти вариации задаются 15 условными кодами.
Также для некоторых инструкций имеется их вариант с суффиксом 'S', который
также имеет 15 вариаций.
Существующими тестами для ARM32 удалось покрыть всего 6% от общего набора
инструкций.
Не удалось покрыть большинство таких вариаций.
Однако все же имеется возможность покрытия такого рода инструкций.
Например, не покрытую инструкцию `addge` (сложить, если истинно условие 'больше
...
Создание тестов, покрывающих такие вариации, выходит за рамки данной
работы.
...
Для ARM32 в таблице представлены не все не покрытые инструкции по причине
...
транслятора TCG эмулятора Qemu и реализовано программное средство на его
основе.
Программное средство находится в открытом доступе.

Проведена оценка покрытия инструкций некоторых процессорных
архитектур, реализованных в Qemu.
Для тестирования были использованы имеющиеся на момент написания работы тесты.
Тем самым показана применимость данного подхода тестирования.

Достоинством предложенного подхода перед существующими решениями является
возможность использовать
в качестве эталона для тестирования персональный компьютер тестировщика.
При этом теряется возможность тестирования
реализаций инструкций, специфичных для конкретного процессора.

Дальнейшим развитием является работа в направлении генерации тестов с
наибольшим покрытием.
Также важность имеют способы принуждения компилятора к использованию
определённых инструкций.
...
[$]. <a name="ref.RISU"></a>https://github.com/stsquad/risu.
