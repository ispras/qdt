...
abstract: |
  ...
  **Аннотация.**
  ...
  Языком был выбран Си, который достаточно близок к
  аппаратуре и для которого имеются компиляторы под большинство процессорных
  архитектур.
...
В свою очередь, при необходимости динамического анализа машинного кода для
узкоспециализированных процессорных архитектур, скорее всего,
готовой виртуальной машины не существует.
Следовательно, возникает потребность в её разработке.
...
использованием системы автоматизации создания фронтенда TCG [[$](#ref.TCGgen)].
...
-->
Под прозрачностью подразумевается успешное сокрытие факта проведения анализа от
анализируемого кода.
...
транслятора.
Данные эталоны используются для проверки правильности выходных значений для
заданного набора входных значений.
...
машинного кода, то есть являются архитектурно-зависимыми.
...
Тем самым достигается возможность использования для сравнения
...
Фронтенд TCG реализован на языке Си.
...
Для оценки данного покрытия Qemu запускалась в режиме пошагового выполнения и с
включенной отладочной трассировкой (`-d exec,nochain`).
...
архитектуры, так и от интересов разработчика.
...
данной
работы подхода было произведено тестирование существующих в "офф-стриме" Qemu
...
покрытию тестированием ветвей фронтенда TCG.
...
инструкций от общего количества инструкций определённой процессорной
архитектуры, реализованной в Qemu.
...
Не покрыты такие инструкции, как циклические сдвиги, некоторые
...
Однако накладывается ограничение на покрытие реализаций
...
компилирования их с определёнными опциями компилятора.

Направлениями дальнейших исследований являются создание тестов с наибольшим
покрытием инструкций тестируемой реализации определённой процессорной
архитектуры, а также автоматизация данного процесса.
