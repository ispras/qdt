...
задач.
В том числе они используются при организации контролируемого окружения для
...
Выбор именно QEMU обычно обусловлен следующими его свойствами:

* открытый исходный код (лицензия GPL);
* поддержка следующих архитектур процессоров в качестве гостевых:
...
* реализация следующих технологий и возможностей, важных с точки зрения
динамического анализа:

    * динамическая двоичная трансляция;
    * интерфейс отладки гостевой среды по протоколу remote GDB
    [[$](#ref.remote_GDB)];
    * захват состояния ВВМ (_snapshot_);
    * сохранение сетевого трафика в PCAP[[$](#ref.PCAP)];
    * программный интерфейс управления ВВМ --- QMP (QEMU Monitor Protocol),
    использующий формат на JSON[[$](#ref.JSON)]

* поддержка следующих операционных систем (ОС) в качестве основных:

    * Microsoft Windows XP и новее,
    * GNU Linux (семейства Debian, Fedora и др.).
    * Mac OS;

* модульная архитектура, допускающая расширение перечня поддерживаемых
архитектур процессоров, ВМ и устройств.

Других развитых эмуляторов, обладающих минимум таким перечнем свойств не
известно.

При динамическом анализе машинного кода для встраиваемых микроконтроллеров,
сетевого оборудования и прочих узкоспециализированных ВМ возникает потребность
реализовать в эмуляторе поддержку соответствующей ВВМ.
...
Данная работа ставит перед собой цель ускорить данный процесс за счёт
выявления и автоматизации рутинных этапов данного процесса.
