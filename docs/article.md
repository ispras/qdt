...
Реализованное API для написания шаблонов в результате представляет собой
модель _гибридного языка_, включающего в себя язык Си и язык
препроцессора.
...
Класс `Type` определяет интерфейс, которого придерживаются его подклассы.
Как можно заметить из перечня подклассов, _тип_ тут понимается как нечто
большее, чем тип в языке Си.
...
Класс `Type` не предусматривает шаблона для генерации типа. Это значит,
...
из других файлов.
Любой тип, кроме `TypeReference` или безымянного указателя (`Pointer`)
может использоваться непосредственно только в файле, где он объявлен.
Если необходимо объявить переменную типа из другого файла, используется
`TypeReference`. Такой подход даёт возможность при генерации кода
однозначно определить следует ли сгенерировать непосредственное
определение типа, или включение заголовочного файла, где он определён.

Тут важно заметить, что в отличие от Си, модель не делает разграничения
на _объявление_ и _определение_. То есть объявление функции в заголовке
...
_переменная_ нечто большее, чем переменная в языке Си. Ниже приводятся
...
Си. Но ссылаться на такой тип указателя по имени запрещено.
Кроме того игнорируется совпадение имён таких типов.

TODO `Macro`, `Initializer`, `Usage`
