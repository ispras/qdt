...
задач.
Одной из задач является организация контролируемого окружения для машинного
кода во время динамического анализа.
Одним из программных средств для организации ВВМ является эмулятор QEMU
[[$](#ref.QEMU)].
На базе этого эмулятора построены такие инструменты динамического анализа как

* ТУДУ;
* Ещё ТУДУ.

Выбор в качестве основы именно QEMU обычно обусловлен следующими его свойствами:

* открытый исходный код (лицензия GPL);
* поддержка разнообразных процессорных архитектур для ВВМ:
...
* работоспособность в окружении следующих операционных систем (ОС):
...
процессорных архитектур, ВМ и устройств.

Других развитых эмуляторов, обладающих как минимум таким перечнем свойств не
известно.
...
На основе указанный параметров инструмент автоматически генерирует заготовки
программного кода моделей, не обладающие законченной функциональностью, но
готовые к компиляции.
Генерация основана на шаблонах --- заготовках программного кода, которые
...
Дальнейшее изложение происходит в следующем порядке:
...
Одним из примеров применения инструмента является платформа Intel Q35. За
основу была взята его реализация в эмуляторе QEMU версии 2.9.5.

В ходе анализа исходного кода QEMU был составлен перечень устройств и
сформулирована их взаимосвязь. Схема ВМ представлена на рисунке [$](#pic.Q35).

![Рисунок <a name="pic.Q35">$</a> --- Схема ВМ Q35](Q35.png)

На этапе подготовки в QEMU вносились изменения, которые могут повлиять на
работу инструмента:

* вынесена в заголовочный файл структура, описывающая устройство MC146818 RTC
и макрос динамического приведения типа к данному устройству;
* добавлены функции инициализации глоабльных переменных slave_pic и isa_pic
для устройства i8259.

На этапе генерации был составлен перечень устройств и их взаимосвязь. Далее
создана модель Q35 в инструменте и сгенерированы заготовки, в том числе и
заготовки для реализации устройств A20 line, port 80, port F0, так как они
были реализованны в коде платформы Q35.

На этапе реализации были сделаны следующие действия:

* добавлен CPU;
* добавлена настройка BIOS;
* реализована модель GSI (Global System Interrupts);
* добавлен режим совместимости MSDOS с исключениями FPU;
* выделена память под eeprom;
* добавлены родительские утсройства к IOAPIC и PCI HOST (в QEMU версии 2.9.5
добавление родительского утсройства в коде платформы используется только в
архитектурах x86_64 и i386)
...
* реализованы A20 line, port 80, port F0;
* скорректированы имёна переменных.

Наличие инициализация устройств на этапе реализации связано с тем, что
некоторые устройства, использующиеся в Q35, реализованы без строгого
соблюдения принятого в QEMU стиля оформления моделей устройств. Приведение
устройств к единому стилю оформления можно вынести в этап подготовки, но
данная работа весьма трудоемка и незначительно влияет на общую оценку
эффективности инструмента, так как на инициализацию перечисленных
устройств потребовалось менее 10 сторк кода.

В таблицах [$](#tbl.Q35_irq_statistic) и [$](#tbl.Q35_statistic) приведены
количественные оценкы объёма работы по реализации Q35 с использованием
инструмента.

На этапе реализации прерывания были разбиты на группы. Это позволило реализовать
модель GSI и произвести регистрацию прерываний в цикле. Оценка объема реализаций
прерываний вынесена в отдельную таблицу [$](#tbl.Q35_irq_statistic), так как
фактически все модификации сгенерированного кода заключаются в замене объявления
порядка 50-ти прерываний (каждое прерывание объявлялось с новой строки) на
несколько массивов с перырваниями, а также заменене последовательной
регистрации прерываний на циклы.

Таблица <a name="tbl.Q35_irq_statistic">$</a>. Оценка объёма реализации
прерываний в Q35
...
Таблица <a name="tbl.Q35_statistic">$</a>. Оценка объёма реализации Q35 без
учета регистрации прерываний
...
Данные собраны вручную с использованием истории коммитов в GIT. Важно учесть,
что изменение строки в данной статистике представлено как 1 удаление и 1
добавление. Таким образом, приблизительно 84% кода платформы было сгенерировано
автоматически без учета регистрации прерываний. С учетом регистрации
прерываний автоматически было сгенерировано 62% кода. И лишь 7% и 30% кода
соответственно пришлось модифицировать.

Итоговая версия платформы Q35 была успешно протестирована. Тестирование
состояло из загрузки ОС Windows7, запуском Internet Explorer с последующим
открытием страницы google.com.
