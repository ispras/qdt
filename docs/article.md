...
будут использоваться имена структур объектов.
...
специальных функций. Свойство является константным значением,
т.е. оно задается при создании устройства и не может быть изменено впоследствии.
...
строковыми именами свойств (требование соглашений о стиле кодирования).
...
структуры объекта.
Наиболее яркий пример применения второг этапа --- свойства.
...
Оболочка полей структуры устройства в свойства усолжняет привычный
...
Условно код модели можно разделить на две части _логическую_ и
_интерфейсную_.
Первая определяет логику работы устройства, а вторая отвечает за взаимодействие
модели устройства с остальной системой посредством фиксированного
API.

Логическая часть обычно сформулирована на естественном языке в документации
на устройство. При этом отсутствует единый формат _формального_ описания,
которого бы придерживались производители.
Ввиду этого автоматизация разработки этой часть весьма затруднительна и
...
была разработана модель. На данный момент не все из них были приведены в соответствие с моделью.
