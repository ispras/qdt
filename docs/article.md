...
Одним из примеров применения инструмента является платформа Intel Q35.
За основу была взята её реализация в эмуляторе QEMU версии 2.9.5.
...
и макрос динамического приведения к типа данного устройства;
* добавлены функции инициализации глобальных переменных `slave_pic` и `isa_pic`,
в которые записываются ссылки два устройства "i8259".
...
В оригинальной ВВМ Q35 эти устройства реализованы вперемешку с кодом машины.
...
Необходимость дополнительной инициализации устройств на этапе реализации
связано с тем, что некоторые устройства реализованы без строгого
соблюдения принятого в QEMU стиля оформления моделей устройств.
Приведение устройств к единому стилю оформления можно было бы вынести в этап
подготовки, но данная работа весьма трудоемка и незначительно влияет на общую
оценку эффективности применения инструмента, так как на инициализацию
перечисленных устройств потребовалось менее 10 строк кода.

На этапе реализации прерывания были разбиты на группы.
Это позволило произвести регистрацию прерываний GSI в цикле.
Оценка объема реализаций прерываний вынесена в отдельную
таблицу [$](#tbl.Q35_irq_statistic).
Фактически все модификации сгенерированного для прерываний кода заключаются
в замене объявлений переменных для порядка 50-ти прерываний
(каждое прерывание объявлялось с новой строки)
на несколько массивов.
Кроме этого, последовательность инструкций связывающая устройства прерываниями
была свёрнута в циклы.

В таблице [$](#tbl.Q35_statistic) приведены
количественные оценки объёма работы по реализации Q35 с использованием
инструмента без работы над прерываниями.
...
Подсчёты выполнены вручную по истории Git.
Важно учесть, что изменение строки в данной статистике представлено как 1
удаление и 1 добавление.
Таким образом, приблизительно 84% кода платформы было сгенерировано
автоматически (без учета регистрации прерываний).
С учетом регистрации прерываний автоматически было сгенерировано 62% кода.
И лишь 7% и 30% кода соответственно пришлось модифицировать.
...
C2600 (CISCO).
За основу была взята его реализация из эмулятора Dynamips [[$](#ref.Dynamips)].
...
выявлена их взаимосвязь в рамках ВМ C2621XM.
Эти данные были сформулированы средствами инструментами в виде схемы.
Она представлена на рисунке [$](#pic.C2621XM).
...
работы по реализации C2621XM с использованием инструмента.
Работа протекала в 3 этапа:
...
работу инструмента (добавлены отсутствующие PCI идентификаторы).
Кроме того был доработан процессор MPC860, а именно:
...
перечень устройств и схема их логического соединения.
Эти данные были внесены в инструмент.
Зетам, с его помощью, были сгенерированы заготовки.
