...
Одним из примеров применения инструмента является машина на базе набора
микроконтроллеров Intel Q35.
За основу была взята её реализация, уже имеющаяся в QEMU версии 2.9.5.
...
Настройка через CLI подразумевает наличие кода, в большой степени зависящего от
конкретной модели устройства.
Генерация такого кода выходит за рамки возможности инструмента.
...
Стоит заметить, что ввиду большого количества прерываний, код ВВМ получился
избыточен.
Это вызвано тем, что в настоящее время инструмент не поддерживает группировку
связанных по смыслу переменных в массивы и инициализацию их в цикле.
Ввиду этого, после этапа реализации была выполнена следующая работа.
Прерывания были разбиты на группы.
Это позволило произвести регистрацию прерываний в цикле.
Изменения, касающиеся прерываний, никак не влияют на
работу самой ВВМ и обеспечивают краткость кода и более наглядное представление
связей между устройствами.
Оценка объема работы с прерываниями вынесена в отдельную
...
Таблица <a name="tbl.Q35_irq_statistic">$</a>. Оценка объёма работы
с прерываниями в Q35
