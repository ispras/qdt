...
Ранее были проведены работы по автоматизации начального этапа разработки машины
[[$](#ref.QDTCommon),[$](#ref.QDTRegisters)].
Побочным продуктом разработанной в течение тех работ инфраструктуры
(QEmu Development Toolkit --- QDT) является расширенная информация о коде
машины,
...
Кроме этого была добавлена поддержка форматов таблиц быстрого поиска имён
...
интересующих разработчика позициях и выполнения определённых действий
программным способом.
Для этого скрипт ставит точки останова, назначая для них функции обратного
вызова.
В условиях непрерывного развития проекта позиции точек останова быстро
становятся недействительными.
Даже небольшое изменение не относящегося к некоторой точке останова кода, но
расположенного выше неё, приводит к сдвигу номера строки.
Это делает скрипт неадекватным.
...
Дело в том, что не только изменение строки может сделать позицию не
актуальной.
Многие строки взаимосвязаны по смыслу.
Поиск метода доказательной проверки актуальности позиции с учётом изменений во
