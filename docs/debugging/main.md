...
Требования к языку скрипта отладчика:
...
MzTake использует существующий язык FrTime.
FrTime --- функциональный язык с lisp-подобным синтаксисом.
...
Захват значений выполняется во время остановки на точке останова.
...
Также в статье приведён пример графического отображения состояния отлаживаемой
программы.
В частности: состояние алгоритма построения минимального остовного дерева.
...
[$]. <a name="ref.MzTake"></a>Marceau, G., Cooper, G.H., Spiro, J.P.,
