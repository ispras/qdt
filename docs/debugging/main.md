...
abstract: |
  ...
  **Аннотация.**
  ...
  в течение которой отладка и корректировка модели циклически следуют друг за
  другом.
...
Разработанное в данной работе средство является расширением для QEmu
Development Toolkit (QDT), направленное на ускорение отладки в последующем
цикле итеративной разработки модели машины.
...
- Возможность для обработчика явно указать, когда нить может быть возобновлена.
...
управление.
Реализация блокировки может быть выполнена с помощью соответствующего
примитива синхронизации при запуске обработчика в отдельной нити или путём
отложенного возврата управления в обработчик, написанный как сопрограмма
(в Python сопрограммы реализуются с помощью т.н. "генераторов").
...
"нижней половины".
"Нижняя половина" будет выполнять оставшиеся, возможно, длительные действия.
...
Так можно отследить
создание и настройку устройства посредством его свойств (подразумеваются
...
программ с учётом обозначенных ограничений.
